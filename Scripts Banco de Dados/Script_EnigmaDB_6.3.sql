...
Tamanho_Texto Decimal(10,2) not null,
...
Duracao_Video Decimal(10,2) not null,
InicioConteudo_Video Decimal(10,2) not null,
FimConteudo_Video Decimal(10,2) not null,
ID_Usuario int not null
)
go
...
ID_Video int null,
ID_Imagem int null,
Ordem_ConteudoTexto int not null,
ID_Usuario int not null
)
go
...
ID_Conteudo int not null,
Descricao_Exercicio varchar(40) not null, 
...
Create Table Nota
(
ID_Nota int identity not null primary key,
ID_Usuario int not null,
ID_Exercicio int not null,
Tentativa_Nota int not null,
Nota_Nota decimal(10,2) not null 
)
go
Create Table Resumo
(
ID_Resumo int identity not null primary key,
ID_Conteudo int not null,
NomeArquivo_Resumo varchar(60) not null,
Arquivo_Resumo Varbinary(Max) not null,
Extensao_Resumo char(4) not null,
ID_Usuario int not null
)
go
Create Table ImagemQuestao
(
ID_ImagemQuestao int identity not null primary key,
ID_Imagem int not null,
ID_Questao int not null,
Ordem_ImagemQuestao int not null,
ID_Usuario int not null 
)
go
...
------------------------------------------------------------------------------------------------------------------------------------
--Tabela Nota
Alter Table Nota add Constraint [FK_Nota_Usuario] foreign key (ID_Usuario) references Usuario(ID_Usuario)
Alter Table Nota add Constraint [FK_Nota_Exercicio] foreign key (ID_Exercicio) references Exercicio(ID_Exercicio)
------------------------------------------------------------------------------------------------------------------------------------
--Tabela Resumo
Alter Table Resumo add Constraint [FK_Resumo_Conteudo] foreign key (ID_Conteudo) references Conteudo(ID_Conteudo)
------------------------------------------------------------------------------------------------------------------------------------
--Tabela ImagemQuestao
Alter Table ImagemQuestao add Constraint [FK_ImagemQuestao_Questao] foreign key (ID_Questao) references Questao(ID_Questao)
Alter Table ImagemQuestao add Constraint [FK_ImagemQuestao_Imagem] foreign key (ID_Imagem) references Imagem(ID_Imagem)
...
@Usuario int
as
Insert Into Exercicio Values (@Conteudo,@Descricao,@Tipo,@AleatorioQuestao,@Usuario)
go
...
@Usuario int
as
Update Exercicio set ID_Conteudo =  @Conteudo,Tipo_Exercicio =  @Tipo,AleatorioQuestao_Exercicio = @AleatorioQuestao,ID_Usuario = @Usuario , Descricao_Exercicio = @Descricao
...
@Usuario int
as
Insert Into Questao Values (@Exercicio,@Ordem,@AleatorioAlternativa,@Pergunta,@Tipo,@Usuario)
go
...
@Usuario int
as
Update Questao set ID_Exercicio = @Exercicio,Ordem_Questao = @Ordem,AleatorioAlternativa_Questao = @AleatorioAlternativa,Pergunta_Questao = @Pergunta,ID_Usuario =  @Usuario, Tipo_Questao = @Tipo
Where ID_Questao = @ID
go
------------------------------------------------------------------------------------------------------------------------------------
-- Tabela Alternativa
...
@InicioConteudo Decimal(10,2),
@FimConteudo Decimal(10,2),
@Usuario int
as
...
@Usuario int
as
Update ConteudoTexto set ID_Conteudo = @Conteudo, ID_Texto = @Texto,ID_Video = @Video,ID_Imagem = @Imagem,Ordem_ConteudoTexto = @Ordem,ID_Usuario = @Usuario
...
@Denuncia bit
as 
Insert Into AvaliacaoPergunta values (@Usuario,@Pergunta,@Avaliacao,@Denuncia)
go
...
@Denuncia bit
as 
Update AvaliacaoPergunta set ID_Usuario = @Usuario,ID_Pergunta = @Pergunta,Avaliacao_AvaliacaoPergunta = @Avaliacao,Denuncia_AvaliacaoPergunta = @Denuncia
...
@Denuncia bit
as 
Insert Into AvaliacaoResposta values (@Usuario,@Resposta,@Avaliacao,@Denuncia)
go
...
@Denuncia bit
as 
Update AvaliacaoResposta set ID_Usuario = @Usuario,ID_Resposta = @Resposta,Avaliacao_AvaliacaoResposta = @Avaliacao,Denuncia_AvaliacaoResposta = @Denuncia
...
@Tentativa int
as
Insert Into Nota values (@Usuario,@Exercicio,@Tentativa,@Nota)
go
create Procedure AlterarNota
@ID int,
@Usuario int,
@Exercicio int,
@Nota decimal(10,2),
@Tentativa int
as
Update Nota set ID_Usuario = @Usuario,ID_Exercicio = @Exercicio, Nota_Nota =  @Nota , Tentativa_Nota = @Tentativa
Where ID_Nota = @ID
go
------------------------------------------------------------------------------------------------------------------------------------
--Tabela Resumo
Create Procedure InserirResumo
@Conteudo int,
...
@Usuario int
as
Insert Into Resumo values (@Conteudo,@NomeArquivo,@Arquivo,@Extensao,@Usuario)
go
Create Procedure AlterarResumo
@ID int,
@Conteudo int,
@NomeArquivo varchar(60),
@Arquivo Varbinary(Max),
...
@Usuario int
as
Update Resumo set ID_Usuario = @Usuario,ID_Conteudo = @Conteudo, Arquivo_Resumo =  @Arquivo, NomeArquivo_Resumo = @NomeArquivo,Extensao_Resumo = @Extensao
Where ID_Resumo = @ID
go
------------------------------------------------------------------------------------------------------------------------------------
--Tabela ImagemResposta
Create Procedure InserirImagemQuestao
@Imagem int,
@Questao int,
@Ordem int,
@Usuario int
as
Insert into ImagemQuestao values (@Imagem,@Questao,@Ordem,@Usuario)
go
Create Procedure AlterarImagemQuestao
@ID int,
@Imagem int,
@Questao int,
@Ordem int,
@Usuario int
as
Update ImagemQuestao set ID_Imagem = @Imagem,ID_Questao = @Questao,Ordem_ImagemQuestao = @Ordem, ID_Usuario = @Usuario
Where ID_ImagemQuestao = @ID
go
------------------------------------------------------------------------------------------------------------------------------------
...
if not exists (select top 1 null from inserted)
	begin 
		select @Item = ID_Nota from deleted
		select @Usuario = ID_Usuario from deleted
		set @Op = 'D'
	end
else
if not exists (select top 1 null from deleted)
	begin 
		select @Item = ID_Nota from inserted
		select @Usuario = ID_Usuario from inserted
		set @Op = 'I'
	end
else
...
Insert Into Auditoria values ('Nota',@Item,@Op,DEFAULT,@Usuario)
go
------------------------------------------------------------------------------------------------------------------------------------
-- Tabela Resumo
Create Trigger AuditoriaResumo
on Resumo
for Insert,update,delete
as
declare
...
if not exists (select top 1 null from inserted)
	begin 
		select @Item = ID_Resumo from deleted
		select @Usuario = ID_Usuario from deleted
		set @Op = 'D'
	end
else
if not exists (select top 1 null from deleted)
	begin 
		select @Item = ID_Resumo from inserted
		select @Usuario = ID_Usuario from inserted
		set @Op = 'I'
	end
else
...
Insert Into Auditoria values ('Resumo',@Item,@Op,DEFAULT,@Usuario)
go
------------------------------------------------------------------------------------------------------------------------------------
-- Tabela ImagemQuestao
Create Trigger AuditoriaImagemQuestao
on ImagemQuestao
for Insert,update,delete
as
declare
...
if not exists (select top 1 null from inserted)
	begin 
		select @Item = ID_ImagemQuestao from deleted
		select @Usuario = ID_Usuario from deleted
		set @Op = 'D'
	end
else
if not exists (select top 1 null from deleted)
	begin 
		select @Item = ID_ImagemQuestao from inserted
		select @Usuario = ID_Usuario from inserted
		set @Op = 'I'
	end
else
...
Insert Into Auditoria values ('ImagemQuestao',@Item,@Op,DEFAULT,@Usuario)
go